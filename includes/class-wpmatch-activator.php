...
class WPMatch_Activator {
	...
	private static function create_tables() {
		...
		// User actions table.
		$table_user_actions = $wpdb->prefix . 'wpmatch_user_actions';
		$sql_user_actions   = "CREATE TABLE IF NOT EXISTS $table_user_actions (
			action_id bigint(20) UNSIGNED NOT NULL AUTO_INCREMENT,
			user_id bigint(20) UNSIGNED NOT NULL,
			target_user_id bigint(20) UNSIGNED NOT NULL,
			action_type enum('like','dislike','super_like','pass','block','report') NOT NULL,
			...
			KEY idx_created (created_at),
			KEY idx_undone (is_undone),
			UNIQUE KEY unique_user_target (user_id, target_user_id, is_undone)
		) $charset_collate;";
		...
		// Match queue table.
		$table_match_queue = $wpdb->prefix . 'wpmatch_match_queue';
		$sql_match_queue   = "CREATE TABLE IF NOT EXISTS $table_match_queue (
			id bigint(20) UNSIGNED NOT NULL AUTO_INCREMENT,
			user_id bigint(20) UNSIGNED NOT NULL,
			potential_match_id bigint(20) UNSIGNED NOT NULL,
			compatibility_score decimal(5,3) DEFAULT 0.500,
			priority int(11) DEFAULT 0,
			last_shown datetime DEFAULT NULL,
			created_at datetime DEFAULT CURRENT_TIMESTAMP,
			PRIMARY KEY (id),
			KEY idx_user_queue (user_id),
			KEY idx_compatibility (compatibility_score),
			KEY idx_priority (priority),
			KEY idx_last_shown (last_shown),
			UNIQUE KEY unique_user_match (user_id, potential_match_id)
		) $charset_collate;";
		...
		dbDelta( $sql_user_actions );
		dbDelta( $sql_matches );
		dbDelta( $sql_match_queue );
	 ...
	 */
	private static function create_revenue_tables() {
		// Include the revenue database class.
		require_once plugin_dir_path( __FILE__ ) . 'class-wpmatch-revenue-database.php';

		// Create revenue tables.
		WPMatch_Revenue_Database::create_tables();
	}
